...
7. **「择世所需」用户未填**，请基于 industry_trends 库 + 用户的 mbti/holland/values 维度组合推断
...
每条必须包含：
1. 职业定义 + 所属行业
2. 四维框架贴合度（爱/长/需/利 各 2-3 句，「需」必须引用上面 industry_trends 数据）
3. 典型职责（5-7 条）
4. 能力门槛
5. 行业前景（**直接引用 industry_trends 中的 growth/drivers/risks**）
6. 薪资范围（结合 targetLocation 给地域对应区间）
7. 匹配度评分（0-100）
  ...
  "careerPaths": [
    {
      ...
      "frameworkAlignment": {
        ...
        "demand": "需的贴合度（必须引用 industry_trends 数据）",
